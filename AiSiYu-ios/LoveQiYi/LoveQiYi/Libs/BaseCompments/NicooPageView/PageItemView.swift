...
class PageItemView: UIView {
    ...
    var isAverageScreen: Bool = false
...
extension PageItemView: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if indexPath.item == currentIndex {
            ...
            cell.itemBtn.titleLabel?.font = UIFont.boldSystemFont(ofSize: 18)
        } else {
            cell.lineView.isHidden = true
            cell.itemBtn.titleLabel?.font = UIFont.boldSystemFont(ofSize: 15)
        }
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        if isAverageScreen {
            let widthAll: CGFloat = self.frame.width - CGFloat(8*(titles.count - 1))
            return CGSize(width: widthAll/CGFloat(titles.count), height: 35)
        }
        let size = self.getwith(font: UIFont.systemFont(ofSize: 15), height: 25, string: titles[indexPath.item])
        return CGSize(width: size.width + 18, height: 35)
    }
