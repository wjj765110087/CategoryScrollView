...
private extension NicooM3u8Parser {
    ...
    func downLoadKeyWith(_ m3u8Content: String) {
        ...
        if !keySttr.isEmpty {
            ...
            if let keyUrl = tryToGetRightKeyPath(keySttr) {
                if let dataKey = try? Data(contentsOf: URL(string: keyUrl)!) {
                    NicooDownLoadHelper.checkOrCreatedM3u8Directory(self.identifier)
                    let filePath = NicooDownLoadHelper.getDocumentsDirectory().appendingPathComponent(NicooDownLoadHelper.downloadFile).appendingPathComponent(self.identifier).appendingPathComponent("key.key")
    ...
    /// 尝试去拼接路径下载key
    func tryToGetRightKeyPath(_ keyLastUrl: String) -> String? {
        for tsUrlHeader in urlTsHeaders {
            var maybeRightUrl = ""
            ...
            if let _ = try? Data(contentsOf: URL(string: maybeRightUrl)!) {
                print("correct key downloadUrl = \(maybeRightUrl)")
                return maybeRightUrl
            } else {
                print("current key download url is not correct.:\(maybeRightUrl)")
            }
        }
        return nil
    }
