...
public protocol NicooPlayerDelegate: class {
    ...
    /// 充值按钮点击
    func authErrorActionClick(chargeHandler: (_ isSuccess: Bool) -> Void)
...
open class NicooPlayerView: UIView {
    ...
    /// 权限提示
    private lazy var authErrorView: NicooAuthErrorView = {
        let authView = NicooAuthErrorView(frame: self.bounds)
        return authView
    }()
...
//        if let tracks = avAsset?.tracks(withMediaType: AVMediaType.video), tracks.count > 0 {
//            print("tracks.count == \(tracks.count)")
//            for track in tracks {
//
//                print("Videosize == \(track.naturalSize)")
//
//            }
//        }
    }
    ...
    private func addUserActionBlock() {
        ...
        playControllViewEmbed.closeButtonClickBlock = { [weak self] (sender) in
            ...
            let isScaleScreenAction = strongSelf.delegate?.scaleScreenAction(strongSelf.isFullScreen!)
            if strongSelf.isFullScreen! {
                if strongSelf.playControllViewEmbed.playLocalFile! {   // 直接全屏播放本地视频
                    ...
                    if isScaleScreenAction ?? false { // 不是全屏操作，改变播放器尺寸
                        strongSelf.playControllViewEmbed.screenLockButton.isHidden = false
        ...
        playControllViewEmbed.fullScreenButtonClickBlock = { [weak self] (sender) in
            ...
            if strongSelf.isFullScreen! {
                ...
                if isScaleScreenAction ?? false { // 不是全屏操作，改变播放器尺寸
                    strongSelf.playControllViewEmbed.screenLockButton.isHidden = true
    ...
    @objc func playToEnd(_ sender: Notification) {
        ...
        if let item = sender.object as? AVPlayerItem {   /// 这里要区分结束的视频是哪一个
            if let asset = item.asset as? AVURLAsset {
                ...
                if let statuModel = delegate?.currentVideoPlayToEnd(model, playControllViewEmbed.playLocalFile!) {
                    showAuthErrorView(statuModel.info)
                } else {
    ...
    // MARK: - 显示权限提示
    private func showAuthErrorView(_ infoMsg: String?) {
        self.addSubview(authErrorView)
        authErrorView.vipTipsTitle.text = infoMsg ?? "开通VIP,无限畅看"
        authErrorView.chargeButtonClickBlock = { [weak self] (sender) in
            guard let strongSelf = self else { return }
            strongSelf.delegate?.authErrorActionClick(chargeHandler: { (paySuccess) in
                if paySuccess {
                    strongSelf.authErrorView.removeFromSuperview()
                }
            })
        }
