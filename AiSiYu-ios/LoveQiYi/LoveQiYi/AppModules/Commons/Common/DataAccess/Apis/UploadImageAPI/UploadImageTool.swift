    ...
    private func request() {
        ...
        Alamofire.upload(multipartFormData: {[weak self] (multipartFormData) in
            ...
            if let image = strongSelf.image {
                let data = UIImage.jpegData(image)(compressionQuality: 0.5)! //UIImage.jpegData(image) as! Data
                let fileN = String(describing: NSDate()) + ".jpg"
                // withName:：是根据文档决定传入的字符串
                multipartFormData.append(data as Data, withName: "image", fileName: fileN, mimeType: "image/jpg")
            }
            if let param = param {
