...
class CommentsMController: QHBaseViewController {
...
// MARK: - 网络请求
extension CommentsMController {
    
    ///用户视频评论列表
    private func loadData() {
        NicooErrorView.removeErrorMeesageFrom(self.view)
        let params = [VideoCommentListApi.kVideo_Id: videoId]
        viewModel.loadVideoCommentListApi(params: params)
    }
    ...
    private func loadNextPage() {
        let params = [VideoCommentListApi.kVideo_Id: videoId]
        viewModel.loadVideoCommentListNextPage(params: params)
    }
    
    
    ///用户评论视频
    private func commentVideo(content: String, videoId: Int) {
        let params: [String : Any] = [VideoCommentApi.kVideo_Id: videoId, VideoCommentApi.kContent: content]
       viewModel.loadVideoCommentApi(params: params)
    }
    /// 用户回复评论
    private func answerComment(content: String, commentId: Int) {
        let params: [String : Any] = [VideoCommentApi.kVideo_Id: videoId, VideoCommentApi.kComment_id: commentId,VideoCommentApi.kContent: content]
        viewModel.loadVideoCommentApi(params: params)
    }
    
    ///用户视频子评论列表
    private func loadCommnetSonData(_ commentModel: VideoCommentModel?) {
        let params = [VideoSonCommentListApi.kVideo_Id: videoId, VideoSonCommentListApi.kComment_id: videoComents[currentSection].id ?? 0, VideoSonCommentListApi.kPageNumber: 1]
        viewModel.loadVideoSonCommentListApi(params: params)
    }
    
    private func loadCommnetSonNextPage(_ commentModel: VideoCommentModel?) {
        let params = [VideoSonCommentListApi.kVideo_Id: videoId, VideoSonCommentListApi.kComment_id: commentModel?.id ?? 0,VideoSonCommentListApi.kPageNumber: commentModel?.answerPageNumber ?? 1]
        viewModel.loadVideoSonCommentListNextPage(params: params)
    }
    
    ///用户视频评论点赞
    private func commentLike(commentId: Int) {
        let params = [VideoCommentLikeApi.kComment_id: commentId]
        viewModel.loadVideoCommentLikeData(params: params)
    }
    ...
    func addCommentViewCallBackHandler() {
        videoCommentView.sendCommentTextHandler = { [weak self] (text) in
            ...
            if let commentId = strongSelf.currentModel?.id {
                ...
                strongSelf.commentVideo(content: text, videoId: strongSelf.videoId)
            }
        }
    }
...
extension CommentsMController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        //videoCommentView.textInputView.becomeFirstResponder()
//        let sonComment = data[indexPath.row]
//        if let comments = sonComment.comment, comments.count > 0 {
//            let model = comments[indexPath.row]
//            if let nikename = model.user?.nikename {
//                videoCommentView.textInputView.text = "@ \(nikename)"
//            }
//        }
    }
}
...
// MARK: - Layout
private extension CommentsMController {
