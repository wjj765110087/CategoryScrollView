...
class VideoExporter {
    ...
    public func export(urlAsset asset: AVURLAsset, completion: @escaping (_ error: Bool, _ message: String) -> Void) {
        ...
        let startTime: CMTime = (self.startTime == nil) ? CMTime.zero : self.startTime!
        ...
        instruction.timeRange = CMTimeRangeMake(start: CMTime.zero, duration: endTime - startTime)
        ...
        if isPortrait {
            ...
            layerInstruction.setTransform(tf2, at: CMTime.zero)
        }
        ...
        if self.volume > 0 {
            if let _: AVAssetTrack = audioTrackTemp {
                ...
                audioMixInputParameters?.setVolumeRamp(fromStartVolume: self.volume, toEndVolume: self.volume, timeRange: CMTimeRangeMake(start: CMTime.zero, duration: mutableComposition.duration))
            }
        }
