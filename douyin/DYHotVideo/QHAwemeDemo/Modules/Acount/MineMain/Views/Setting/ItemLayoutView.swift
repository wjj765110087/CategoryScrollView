...
class ItemLayoutView: UIView {
    ...
    func viewLayoutWith(aTitle: String, aArray: [ItemModel]) {
        ...
        for (index,value) in aArray.enumerated() {
            ...
            let colorNormal = UIColor(red: 56/255.0, green: 59/255.0, blue: 71/255.0, alpha: 0.99)
            ...
            buton.setBackgroundImage(UIImage.imageFromColor(ConstValue.kTitleYelloColor, frame: CGRect(x: 0, y: 0, width: 120, height: 30)), for: .selected)
            buton.setTitleColor(UIColor.white, for: .normal)
            buton.setTitleColor(UIColor.darkText, for: .selected)
    ...
    @objc func buttonAction(sender: UIButton) {
        if sender.isSelected { // 已经选中
            ...
            if !buttons.contains(sender) && buttons.count < 3 {
                ...
                DLog("allrealdy chose")
            }
        }
        titleText = "请选择问题出现场景\(buttons.count)/3（必选)"
       // DLog("buttons = \(buttons)")
