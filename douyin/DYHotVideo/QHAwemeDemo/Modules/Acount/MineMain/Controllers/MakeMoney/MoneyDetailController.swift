...
class MoneyDetailController: QHBaseViewController {
    ...
    private lazy var balanceDetailApi: UserBalanceDetailApi = {
       let api = UserBalanceDetailApi()
       api.paramSource = self
       api.delegate = self
       return api
    }()
    
    var models: [MoneyDetailModel] = [MoneyDetailModel]()
    ...
    func loadData() {
        ...
        let _ = balanceDetailApi.loadData()
    }
    
    func loadNextpage() {
        let _ = balanceDetailApi.loadNextPage()
    }
...
extension MoneyDetailController: UITableViewDataSource, UITableViewDelegate {
...
extension MoneyDetailController: NicooAPIManagerCallbackDelegate, NicooAPIManagerParamSourceDelegate {
    ...
    func managerCallAPISuccess(_ manager: NicooBaseAPIManager) {
        ...
        if  manager is UserBalanceDetailApi {
            if let balanceDetailList = manager.fetchJSONData(UserReformer()) as? MoneyDetailList {
                if let models = balanceDetailList.data, let currentPage = balanceDetailList.current_page {
                    if currentPage == 1 {
                        self.models = models
                        if models.count == 0 {
                    ...
                    loadMoreView.isHidden = models.count < UserBalanceDetailApi.kDefaultCount
                }
...
extension MoneyDetailController:  QHNavigationBarDelegate  {
...
// MARK: - Description
private extension MoneyDetailController {
