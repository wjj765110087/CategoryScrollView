    ...
    func addUserInfoViewModelCallBack() {
        userInfoViewModel.followAddOrCancelSuccessHandler = { [weak self] isAdd , model in
            ...
            if strongSelf.videos.count > 0 {
                ...
                strongSelf.collection.reloadItems(at: [IndexPath(item: strongSelf.currentIndex, section: 0)])
                ///通知用户主页去再次请求关注状态的接口
...
extension AcountVideoPlayController: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.talksKeyClickHandler = { [weak self] in
            guard let strongSelf = self else { return }
            if let model = videoModel.topic?.topic {
...
// MARK: - play video
extension AcountVideoPlayController {
    private func configVideo(_ videoModel: VideoModel, cell: PresentPlayCell, indexPath: IndexPath) {
        if (videoModel.is_coins ?? 0) == 1 {                // 金币视频
            ...
            playerView.setCoinsVideoValues(isCoin: false, coinsCount: 0, coinsUser: 0, userNickName: videoModel.user?.nikename ?? "")
...
extension AcountVideoPlayController: NicooPlayerDelegate, NicooCustomMuneDelegate {
