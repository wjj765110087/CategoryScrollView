...
class PhoneFindAcountController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        loginView.loginBtn.setTitle("立即切换", for: .normal)
    }
    ...
    private func showAlert(_ success: Bool, errorMsg: String?) {
        let succeedModel = ConvertCardAlertModel.init(title: "恭喜切换成功", msgInfo: "您的账号信息已切换。", success: true)
        let failedModel = ConvertCardAlertModel.init(title: "很遗憾切换失败", msgInfo: errorMsg ?? "请稍后再试", success: false)
...
extension PhoneFindAcountController:  QHNavigationBarDelegate  {
    func backAction() {
