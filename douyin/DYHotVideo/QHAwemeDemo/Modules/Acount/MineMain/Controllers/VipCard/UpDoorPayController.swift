...
/// 上门服务
class UpDoorPayController: UIViewController {
    ...
    private lazy var tableView: UITableView = {
        ...
        table.register(UpdoorServerContentCell.classForCoder(), forCellReuseIdentifier: UpdoorServerContentCell.cellId)
    ...
    private lazy var vipCardApi: UpDoorServerApi = {
        let api = UpDoorServerApi()
        api.delegate = self
    ...
    var bottomViewServerHandler:((_ paytypeId: Int, _ vipCardModel: UpDoorServerModel)->())?
...
extension UpDoorPayController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        if indexPath.section == 0 {
            return UpdoorServerContentCell.cellHeight
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 0 {
            let cell = tableView.dequeueReusableCell(withIdentifier: UpdoorServerContentCell.cellId, for: indexPath) as! UpdoorServerContentCell
            cell.setModels(serverModels)
            cell.itemClickHandler = { [weak self] index in
                guard let strongSelf = self else { return }
                strongSelf.selectedIndex = index
                strongSelf.loadPayTypes()
                strongSelf.bottomViewServerHandler?(1, strongSelf.serverModels[index])
            }
            return cell
        } else if indexPath.section == 1 {
...
extension UpDoorPayController: NicooAPIManagerCallbackDelegate, NicooAPIManagerParamSourceDelegate {
    
    func paramsForAPI(_ manager: NicooBaseAPIManager) -> [String : Any]? {
        XSProgressHUD.showCustomAnimation(msg: nil, onView: view, imageNames: nil, bgColor: nil, animated: false)
        if manager is UserPayTypeListApi {
            let model = serverModels[selectedIndex]
        ...
        if manager is UserOrderAddApi {
            ...
            params[UserOrderAddApi.kVc_id] = serverModels[selectedIndex].id
            if currentPayType != nil {
    ...
    func managerCallAPISuccess(_ manager: NicooBaseAPIManager) {
        ...
        if manager is UpDoorServerApi {
            if let serList = manager.fetchJSONData(UserReformer()) as? UpDoorServerListModel {
                if let list = serList.service, let noticeMsg = serList.service_message {
                    serverModels = list
                    self.noticeMsg = noticeMsg
                    loadPayTypes()
                    ///默认选中第1个
                    bottomViewServerHandler?(1, serverModels[0])
...
// MARK: - Layout
private extension UpDoorPayController {
