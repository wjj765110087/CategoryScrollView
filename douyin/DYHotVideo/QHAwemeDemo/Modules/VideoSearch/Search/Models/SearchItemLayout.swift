...
class SearchItemLayout: UICollectionViewFlowLayout {
    override func prepare() {
        super.prepare()
        //cell数据离头距离
        self.sectionInset = UIEdgeInsets(top: 5, left: 15, bottom: 10, right: 15)
        //每组头的大小
    ...
    override func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        for i in 1 ..< arrCell!.count {
            ...
            if (prevLayout.indexPath.section == currentLayout.indexPath.section) {
                ...
                if i == 1 {
                    var frameV = prevLayout.frame
                    frameV.origin.x = self.sectionInset.left
                    prevLayout.frame = frameV
                }
...
class CateItemLayout: UICollectionViewFlowLayout {
    override func prepare() {
        super.prepare()
        //cell数据离头距离
        self.sectionInset = UIEdgeInsets(top: 5, left: 15, bottom: 10, right: 15)
        //每组头的大小
        self.headerReferenceSize = CGSize(width: UIScreen.main.bounds.width, height: 0)
        self.minimumInteritemSpacing = 15
    }
