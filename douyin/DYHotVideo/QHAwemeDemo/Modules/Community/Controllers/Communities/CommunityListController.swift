...
/// 社区内容页
class CommunityListController: QHBaseViewController {
    ...
    override func viewDidLoad() {
        ...
        layoutPageSubviews()
        tableView.contentInset = UIEdgeInsets(top: UIDevice.current.isiPhoneXSeriesDevices() ? 20 : 10, left: 0, bottom: UIDevice.current.isiPhoneXSeriesDevices() ? 88 : 75, right: 0)
...
// MARK: - Private - Funcs
private extension CommunityListController {
    
    func getparams()-> [String : Any] {
        let params: [String: Any] = [UserTopicListApi.kIs_recommend: isRecommend ? 1 : 0, UserTopicListApi.kIs_attention: isAttention ? 1 : 0, UserTopicListApi.kUserId: -1]
        return params
    }
    func loadTopicListData() {
        NicooErrorView.removeErrorMeesageFrom(self.view)
        XSProgressHUD.showCustomAnimation(msg: nil, onView: view, imageNames: nil, bgColor: nil, animated: false)
        viewModel.loadUserTopicList(getparams())
    }
    func loadFirstPage() {
        viewModel.loadUserTopicList(getparams())
    }
    func loadNextPage() {
        viewModel.loadUserTopicListNextPage(getparams())
    }
    func endRefreshing() {
        ...
        if let dataList = listModels {
            if pageNumber == 1 {
                ...
                if dataList.count == 0 {
                    ...
                    , isAttention ? "你还没有关注过别人\n快去关注吧" : "暂无精选动态\n快去发布吧", on: view, topMargin: 0) {
    ...
    private func getCurrentVC() -> CommunityMainController? {
        var next = view.superview
        while (next != nil) {
            let nextResponder = next?.next
            if (nextResponder is CommunityMainController) {
                return nextResponder as? CommunityMainController
            }
            next = next?.superview
        }
        return nil
...
extension CommunityListController: UITableViewDataSource, UITableViewDelegate {
        ...
        if model.type == TopicType.imgText {
            if let imageResource = model.resource {
                if imageResource.count == 0 {
                    ...
                    cell.setModel(model,isAttention ? .attention : .recomment)
...
extension CommunityListController: UIScrollViewDelegate {
...
private extension CommunityListController {
    ...
    func layoutTopButton() {
        topButton.snp.makeConstraints { (make) in
            ...
            make.bottom.equalTo(UIDevice.current.isXSeriesDevices() ? -140 : -100)
...
extension UIViewController {
    func fixModelFollowStatu(_ statu: TopicRecommend, userId: Int, datas: [TopicModel]) -> [TopicModel] {
        for topic in datas {
            if topic.user?.id == userId {
                topic.is_attention = statu
                topic.user?.followed = (statu == .recommend ? .focus : .notFocus)
            }
        }
        return datas
    }
