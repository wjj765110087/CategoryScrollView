...
public class YQLabel: UILabel {
    ...
    fileprivate var drawText: NSAttributedString? {
        var text = texts.reduce(NSMutableAttributedString()) { (result, paragraph) -> NSMutableAttributedString in
            ...
            if let backColor = paragraph.backGColor {
                attributes[NSAttributedString.Key.backgroundColor] = backColor
            } else {
            ...
            if let touchedParagraph = self.touchedParagraph {
                if isTouching && nil != paragraph.clickHandler && touchedParagraph == paragraph{
                    attributes[NSAttributedString.Key.backgroundColor] = highlightedColor
                }
            }
        ...
        text = NSMutableAttributedString(string: string, attributes: [NSAttributedString.Key.font: font, NSAttributedString.Key.foregroundColor: textColor])
        return text
    }
    
    fileprivate var rects: [YQRect : StringParagraph] = [:]
    
    public func add(text: String, tag: Int = Int.max, color: UIColor? = nil, backColor: UIColor? = nil, clickHandler: YQTextClickHandler? = nil) {
        var location: Int = 0
        if let last = texts.last {
            location = last.range().upperBound
        }
        texts.append(StringParagraph(text: text, tag: tag, color: color, backGColor: backColor, clickHandler: clickHandler, location: location))
    }
