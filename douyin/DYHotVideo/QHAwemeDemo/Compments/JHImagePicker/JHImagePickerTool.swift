...
///   - asset: 图片资源
///   - finished: 图片回调
func getOriginalData(_ asset: PHAsset, finished: @escaping ((_ originalData: Data) -> Void )) {
    let manager = PHCachingImageManager()
    let options = PHImageRequestOptions()
    options.isSynchronous = true
    manager.requestImageData(for: asset, options: options, resultHandler: { data, str, _, info in
