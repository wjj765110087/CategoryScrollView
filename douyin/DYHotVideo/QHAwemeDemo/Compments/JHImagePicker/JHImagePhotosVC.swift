...
class JHPhotoItem {
    ...
    // 获取原图二进制Data
    func originalData(_ finished: @escaping (_ originData: Data) -> Void) {
        if let ass = self.asset {
            getOriginalData(ass) { (data) in
                finished(data)
            }
        }
    }
...
class JHImagePhotosVC: UIViewController {
    ...
    func scrollToBottom() {
        if let asset = assets {
            if asset.count <= 1 || !isFirstLoad { return }
            isFirstLoad = false
            
            let indexPath = IndexPath(row: asset.count - 1, section: 0)
    ...
    private func loadCollectionView() {
        ...
        let f = CGRect(x: 0, y: safeAreaTopHeight, width: view.bounds.width, height: view.bounds.height - safeAreaTopHeight - TabbarSafeBottomMargin()-50)
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let asset = item.asset {
            ...
            if item.imageData == nil {
                imageManager.requestImageData(for: asset, options: options, resultHandler: { data, str, _, info in
                    print("data.count == \(data?.count ?? 0)")
                    item.imageData = data
                })
            }
            if let fileName = asset.value(forKey: "filename") as? String {
                if asset.mediaType == .image && fileName.hasSuffix("GIF") {
                    print("这是一张gif图片")
